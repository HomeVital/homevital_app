...
const BloodPressure = (): JSX.Element => {
	...
	const cardItem = (item: IOxygenSaturation) => {
		return (
	...
	const GraphView = () => {
		...
		const filteredData = Object.keys(dataTypes).map((key) =>
			data?.map((item) => ({ value: Number(item[key as keyof IOxygenSaturation]) })),
		);
		...
		return (
			<View style={Styles.container}>
				...
				{focusIndex !== undefined && cardItem(item as IOxygenSaturation)}
			</View>
		);
	};
